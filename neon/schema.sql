-- ============================================================================
-- SELVAKKODI AGRO SERVICES / THENN NADU BILLING
...
-- Ensures legacy or helper calls to auth.uid() or auth.jwt() execute cleanly in Neon
...
-- 4. USERS TABLE (Replaces Supabase Auth users)
...
BEGIN
  ...
  RETURN QUERY SELECT * FROM public.complete_advance_order_v2(
    p_order_id, p_payment_method, COALESCE(v_remaining, 0), NULL, 0, 0, p_remarks
  );
END;
$$;

-- Trigger to deduct inventory stock on manual order status changes to 'completed'
