  ...
  return (
    ...
    >
      ...
      <div style={{ textAlign: 'center', borderBottom: '1px solid #e5e7eb', paddingBottom: 20, marginBottom: 20 }}>
        <div style={{ width: 80, height: 80, margin: '0 auto 12px auto', background: '#ffffff', borderRadius: 16, border: '1px solid #A5D6A7', padding: 6, boxShadow: '0 4px 12px rgba(0,0,0,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
          <img src={BRAND_LOGO} alt={`${BRAND_EN} logo`} style={{ width: '100%', height: '100%', objectFit: 'contain', display: 'block' }} />
        </div>
        <div style={{ fontSize: 22, fontWeight: 900, color: '#2E7D32', letterSpacing: -0.5, textTransform: 'uppercase' }}>
          {BRAND_EN}
        </div>
        <div style={{ fontSize: 13, fontWeight: 700, color: '#1B5E20', marginTop: 2 }}>
          {BRAND_OWNER}
        </div>
        <div style={{ fontSize: 11, color: '#4b5563', marginTop: 4, fontWeight: 500, paddingLeft: 8, paddingRight: 8 }}>
          {BRAND_ADDRESS}
        </div>
        <div style={{ fontSize: 11, color: '#4b5563', marginTop: 4, display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
        ...
        <div style={{ minWidth: 0, padding: '12px 14px', borderRadius: 10, background: '#E8F5E9', overflowWrap: 'anywhere' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 320 }}>
          <thead>
            <tr style={{ background: '#f3f8f3', borderRadius: 8 }}>
              ...
              <th style={{ padding: '8px 10px', textAlign: 'left', fontSize: 10, fontWeight: 800, color: '#2E7D32', textTransform: 'uppercase', letterSpacing: 0.8 }}>Product</th>
          ...
          <div style={{ minWidth: 240, width: '100%', maxWidth: 300 }}>
            ...
            {discountAmount > 0 && (
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                ...
                <span style={{ fontSize: 12, fontWeight: 700, color: '#2E7D32' }}>−{formatCurrency(discountAmount)}</span>
              </div>
            )}
            {manualDiscountAmount > 0 && (
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                <span style={{ fontSize: 12, color: '#2E7D32' }}>Manual Discount</span>
                <span style={{ fontSize: 12, fontWeight: 700, color: '#2E7D32' }}>−{formatCurrency(manualDiscountAmount)}</span>
              </div>
            )}
            {gstAmount > 0 && (
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
            ...
            {effectiveDelivery === 0 && discountAmount === 0 && manualDiscountAmount === 0 && gstAmount === 0 && (
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                <span style={{ fontSize: 12, color: '#666' }}>Delivery</span>
                <span style={{ fontSize: 12, fontWeight: 700, color: '#2E7D32' }}>FREE</span>
              </div>
            )}
            <div
              style={{
            ...
            >
              <span style={{ fontSize: 15, fontWeight: 900, color: '#2E7D32', textTransform: 'uppercase', letterSpacing: 0.5 }}>Total</span>
              <span style={{ fontSize: 20, fontWeight: 900, color: '#2E7D32' }}>{formatCurrency(total)}</span>
            </div>
          </div>
        </div>
      </div>
