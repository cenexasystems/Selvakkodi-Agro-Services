...
export default function CatalogModal({ isOpen, onClose, onAdd }: CatalogModalProps) {
  ...
  const saveEdit = async (e: React.FormEvent) => {
    ...
    if (error) { setEditError(error); setEditLoading(false); return }
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl sm:rounded-3xl w-full max-w-5xl flex min-h-0 flex-col shadow-2xl overflow-hidden border border-[#A5D6A7]/40 max-h-[calc(100dvh-1rem)] sm:max-h-[85vh]">
        ...
        {editingProduct ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={editForm.category}
                    ...
                    className="w-full min-w-0 h-12 px-4 py-3 bg-[#F9FAFB] border border-[#A5D6A7]/60 rounded-xl focus:outline-none focus:border-[#2E7D32] text-[13px] font-bold touch-manipulation">
                ...
                <div>
                  ...
                  <input type="number" value={editForm.price}
                    ...
                    className="w-full px-4 py-3 bg-[#F9FAFB] border border-[#A5D6A7]/60 rounded-xl focus:outline-none focus:border-[#2E7D32] text-[13px] font-bold text-right" placeholder="0" />
                </div>
              </div>
          ...
          <>
            <div className="flex items-center justify-between p-5 border-b border-[#A5D6A7]/40 bg-[#F9FAFB]">
            ...
            <div className="p-3 sm:p-4 border-b border-[#A5D6A7]/40 bg-white space-y-3">
              <div className="relative">
                ...
                <input type="text" value={search}
                  ...
                  className="w-full pl-10 pr-4 py-3 bg-[#FAFAFA] border border-[#A5D6A7]/60 rounded-xl focus:outline-none focus:border-[#2E7D32] text-[13px] font-bold text-[#111111]" />
              </div>
              <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
                {categories.map(cat => (
                  <button key={cat} onClick={() => setActiveCategory(cat)}
                    className={`px-4 py-2 rounded-xl text-[11px] font-black uppercase tracking-wider whitespace-nowrap transition-colors ${activeCategory === cat ? 'bg-[#2E7D32] text-white' : 'bg-[#FAFAFA] text-[#374151] hover:bg-[#F9FAFB] border border-[#A5D6A7]/60'}`}>
                    {cat}
                  </button>
                ))}
              </div>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto p-3 sm:p-4 bg-[#FAFAFA]">
              {loading ? (
                <div className="flex min-h-48 flex-col items-center justify-center gap-3 text-[#374151]/70">
                  <span className="h-7 w-7 animate-spin rounded-full border-2 border-[#A5D6A7] border-t-[#2E7D32]" />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                  {filtered.map(product => (
                    <div key={product.id}
                      className="bg-white border border-[#A5D6A7]/60 rounded-2xl p-3 flex flex-col gap-2 hover:border-[#2E7D32]/40 hover:shadow-md transition-all group relative">
                      <div className="flex justify-end gap-1 mb-1 md:absolute md:top-2 md:right-2 md:opacity-0 md:group-hover:opacity-100 transition-opacity z-10">
                        <button onClick={(e) => { e.stopPropagation(); startEdit(product) }} title="Edit product"
                          className="p-1.5 rounded-lg bg-white border border-[#A5D6A7]/60 text-[#374151] hover:text-[#2E7D32] hover:border-[#2E7D32]/40 shadow-sm transition-colors">
                          <Edit2 size={14} />
                        </button>
                        <button onClick={(e) => { e.stopPropagation(); void handleDelete(product) }} title="Delete product"
                          className="p-1.5 rounded-lg bg-white border border-[#A5D6A7]/60 text-red-400 hover:text-red-600 hover:border-red-300 shadow-sm transition-colors">
                          <Trash2 size={14} />
                        </button>
                      </div>
                      <div onClick={() => onAdd(product)} className="cursor-pointer flex-1">
                        <div className="flex items-start justify-between gap-2">
                          <div className="min-w-0">
                            <h4 className="text-[13px] font-black text-[#111111] leading-tight group-hover:text-[#2E7D32] transition-colors truncate">{product.name}</h4>
                      ...
                      <div onClick={() => onAdd(product)} className="cursor-pointer">
                        <div className="flex items-center justify-between mt-2 pt-2 border-t border-[#A5D6A7]/30 gap-2">
                          ...
                          <div className="flex items-center justify-end gap-1.5 overflow-hidden">
                            ...
                            <span className="text-[9px] font-black text-[#374151] uppercase tracking-wider bg-[#F9FAFB] px-2 py-1 rounded border border-[#A5D6A7]/40 truncate min-w-0">{product.category}</span>
                          </div>
                        </div>
                      </div>
                    </div>
