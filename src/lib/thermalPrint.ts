    ...
    <html>
      ...
      <body>
        <div class="text-center mb-2">
          <div style="margin-bottom: 6px;"><img src="${BRAND_LOGO}" alt="${BRAND_EN}" style="width: 50px; height: 50px; object-fit: contain; display: block; margin: 0 auto; border-radius: 8px;" /></div>
          <div class="font-bold" style="font-size: 16px;">${data.storeName || BRAND_EN}</div>
          <div class="font-bold" style="font-size: 12px; color: #1B5E20; margin-top: 2px;">${BRAND_OWNER}</div>
          <div style="font-size: 10px; margin-top: 2px;">${data.storeAddress || BRAND_ADDRESS}</div>
          <div class="mt-1" style="font-size: 10px;">Ph: ${data.storePhone || BRAND_PHONE_DISPLAY}</div>
          <div style="font-size: 10px;">Email: ${BRAND_EMAIL}</div>
        </div>
