...
const STATUS_COLORS: Record<string, { bg: string; text: string; label: string }> = {
  ...
  completed: { bg: '#A5D6A7', text: '#065F46', label: 'Completed' },
...
export default function Profile() {
  ...
  const handleAvatarUpload = async (file: File) => {
    ...
    try {
      ...
      const { error } = await authService.updateProfile({ avatar_url: url })
      if (!error) {
        setAvatarUrl(url)
  ...
  const handleSaveProfile = async () => {
    ...
    if (trimPhone && !isValidPhone(trimPhone)) { setSaveErr('Enter a valid 10-digit mobile number.'); return }
    if (!user) return

    setSaving(true); setSaveErr('')
    const { error } = await authService.updateProfile({ name: trimName, mobile: trimPhone })
