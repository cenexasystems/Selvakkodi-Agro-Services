...
import { formatCurrency, formatBillDateTime } from '../lib/retail'
...
const RMIcon = ({ size = 20, className = '' }: { size?: number; className?: string }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className={className} aria-label="Indian Rupee">
    ...
    <text x="12" y="16" textAnchor="middle" fontSize="11" fontWeight="bold" stroke="none" fill="currentColor" fontFamily="Arial, sans-serif">₹</text>
  </svg>
)
...
export function formatDeliveryDate(dateStr?: string | null): string {
  if (!dateStr) return '—'
...
export default function AdvanceOrders({ onOrderCompleted }: AdvanceOrdersProps = {}) {
  ...
  useEffect(() => {
    couponService.fetchCoupons()
      .then(({ data }) => {
        if (data) setAvailableCoupons(data.map(c => ({ code: c.code, percentage: c.percentage })))
      })
  }, [])
  ...
  const applyCoupon = async () => {
    ...
    if (!code) return
    const balance = paymentOrder ? paymentOrder.remaining_balance : 0
    const { data, error } = await couponService.validateCoupon(code, balance)
    if (error || !data) {
      setCouponError(error || 'Invalid or inactive coupon code.')
      return
    }
  ...
  const deleteOrder = async (order: AdvanceOrder) => {
    if (!window.confirm(`Delete advance order "${order.deposit_id}" for ${order.customer_name}? This cannot be undone.`)) return
    try {
      const token = localStorage.getItem('selvakkodi_auth_token') || localStorage.getItem('selvakkodi-admin-token') || localStorage.getItem('purple-boutique-token')
      const res = await fetch(`/api/advance-orders/${order.id}`, {
        method: 'DELETE',
      ...
      setNotice(`Advance order ${order.deposit_id} deleted.`)
