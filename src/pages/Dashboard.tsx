...
type TabKey = 'overview' | 'whatsapp' | 'pos_analytics' | 'billing' | 'advance_orders' | 'coupons' | 'users' | 'history' | 'expenses' | 'attendance' | 'inventory' | 'settings'
...
const displayPhone = (phone: any): string => {
  if (!phone) return '-'
...
const UNIT_TYPE_OPTIONS: { value: UnitType; label: string; hint: string }[] = [
  { value: 'unit',   label: 'Unit (piece)',    hint: 'e.g. Seeds pack, equipment, sprayers' },
  { value: 'weight', label: 'Weight (g / kg)', hint: 'For weight-based seeds or fertilizers' },
  { value: 'volume', label: 'Volume (ml / L)', hint: 'e.g. Liquid pesticides, nutrients' },
  { value: 'bundle', label: 'Bundle / Set',    hint: 'e.g. Farm kit, combo pack' },
]
...
export default function Dashboard() {
  ...
  const { settings, fetchSettings: reloadSettings } = useSettingsStore()
  const globalLowStockLimit = settings?.lowStockLimit ?? 5

  useEffect(() => {
    if (products.length > 0) {
      inventoryAlertService.checkProducts(products, globalLowStockLimit)
    }
  }, [products, globalLowStockLimit])
  ...
  useEffect(() => {
    if (settings) {
      setSettingsForm({
        name: settings.name || 'Selvakkodi Agro Service',
        ownerName: settings.ownerName || 'S. Rajesh Kumar',
        phone: settings.phone || '9080788263 & 9677791900',
        email: settings.email || 'selvakkodiagroservice@gmail.com',
        address: settings.address || 'No.499/A, Thanipadi Main Road, Thandarampattu, Tiruvannamalai District, Tamil Nadu, PIN 606707',
        instagram: settings.instagram || 'selvakkodi_agro_service',
  ...
  const handleSaveSettings = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await settingsService.updateSettings(settingsForm)
      if (error) {
        setSettingsSaveError(error)
      } else {
        setSettingsSaveSuccess('Store configuration updated successfully!')
        await reloadSettings()
      }
    } catch (err: any) {
      setSettingsSaveError(err.message || 'Failed to update settings')
    } finally {
  ...
  const lowStockProducts = products.filter(p => {
    ...
    const limit = Number(p.lowStockAlert ?? globalLowStockLimit)
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const [cRes, ordersData, couponRes, eRes] = await Promise.all([
        categoryService.fetchCategories(),
        orderService.getOrders({ limit: 1000, includeItems: true }),
        couponService.fetchCoupons(),
        expenseService.fetchExpenses(),
      ])
      if (cRes.error) throw cRes.error
      const mappedOrders = (ordersData || []).map((r: any) => toDashboardOrder(r as Record<string, unknown>))
      ...
      const allItems = (ordersData || []).flatMap((o: any) => (o.order_items || []).map((r: any) => ({
        order_id: String(r.order_id || o.id || ''),
        product_name: String(r.product_name || r.name || 'Product'),
        category: String(r.category || ''),
        quantity: toNumber(r.quantity, 0),
        line_total: toNumber(r.line_total, 0),
  ...
  const loadUsers = useCallback(async () => {
    ...
    const { data, error } = await authService.fetchUsers()
  ...
  const loadCoupons = useCallback(async () => {
    const { data, error } = await couponService.fetchCoupons()
    if (!error && data) {
  ...
  const toggleUserRole = async (u: ProfileUser) => {
    ...
    const { error } = await authService.updateUserRole(u.id, newRole)
    if (!error) {
  ...
  const generateCouponCode = () => {
    const prefixes = ['SAVE', 'AGRO', 'FARM', 'SHOP', 'SPECIAL', 'GROW', 'NEW']
  ...
  const saveCoupon = async (e: FormEvent) => {
    ...
    if (editingCouponId !== null) {
      const res = await couponService.updateCoupon(editingCouponId, payload)
      error = res.error
    } else {
  ...
  const runSearch = async (e?: FormEvent, dateFromOverride?: string, dateToOverride?: string, billTypeOverride?: string) => {
    ...
    try {
      ...
      let orderTypeParam: string | undefined = undefined
      let orderModeParam: string | undefined = undefined
      if (effectiveBillType === 'manual') orderTypeParam = 'manual_sale'
      else if (effectiveBillType === 'offline') { orderTypeParam = 'pos_sale'; orderModeParam = 'offline' }
      else if (effectiveBillType === 'online')  { orderTypeParam = 'pos_sale'; orderModeParam = 'online' }

      const searchResultsData = await orderService.getOrders({
        invoiceNo: invInput || undefined,
        phone: phoneInput || undefined,
        customerName: custInput || undefined,
        startDate: effectiveDateFrom ? `${effectiveDateFrom}T00:00:00` : undefined,
        endDate: effectiveDateTo ? `${effectiveDateTo}T23:59:59` : undefined,
        orderType: orderTypeParam,
        orderMode: orderModeParam,
        limit: hasQuery ? 1000 : 500,
      })

      let results = (searchResultsData || [])
        .filter((r: any) => normalizeOrderType(r.order_type) !== 'online_request')
        .map((r: any) => toDashboardOrder(r as Record<string, unknown>))
  ...
  const handleToggleActive = async (p: Product) => {
    const { error } = await updateProduct(p.id, { is_active: !p.isActive })
    if (error) { setProductNotice(error); return }
    ...
    try {
      ...
      if (editingVariantId) {
        ...
        if (!editingProd.hasVariants) {
          await updateProduct(editingProd.id, { has_variants: true })
        }
      }
      setVariantForm({ name: '', sizeLabel: '', price: '', purchasePrice: '', mrp: '', sku: '', barcode: '', stock: '50', weightValue: '', weightUnit: '', isDefault: false })
      setEditingVariantId(null)
  ...
  const onAddCat = async (e: FormEvent) => {
    ...
    const { error } = editingCategoryId === null
      ? await categoryService.createCategory({ ...payload, is_active: true })
      : await categoryService.updateCategory(editingCategoryId, payload)
    if (error) {
      setCategoryNotice({ type: 'error', text: error || 'Could not add category.' })
      return
    }
  ...
  const deleteCat = async (c: Category) => {
    ...
    const { error } = await categoryService.deleteCategory(c.id)
    if (error) {
      setCategoryNotice({ type: 'error', text: error || 'Could not delete category.' })
      return
    }
  ...
  const toggleCat = async (c: Category) => {
    ...
    const { error } = await categoryService.updateCategory(c.id, { is_active: !c.is_active })
    if (error) {
  ...
  const moveCat = async (c: Category, dir: 'up' | 'down') => {
    ...
    if (dir === 'up' && currentIndex > 0) {
      ...
      await categoryService.reorderCategories([
        { id: c.id, sort_order: currentNormalized.sort_order },
        { id: prevCat.id, sort_order: prevNormalized.sort_order }
      ])
      ...
      await categoryService.reorderCategories([
        { id: c.id, sort_order: currentNormalized.sort_order },
        { id: nextCat.id, sort_order: nextNormalized.sort_order }
      ])
    }
  }
  ...
  const allNavItems: Array<{ id: TabKey; icon: React.ReactNode; label: string }> = [
    ...
    { id: 'settings',      icon: <Settings size={20} />,         label: 'Store Settings' },
  ]
  const navItems = allNavItems.filter(item => role === 'admin' || (item.id !== 'pos_analytics' && item.id !== 'coupons' && item.id !== 'expenses' && item.id !== 'settings'))
      ...
      <main className="flex-grow flex flex-col overflow-hidden">
        ...
        {tab === 'overview' && (() => {
          ...
          <div className="space-y-6 rounded-[28px] bg-maroon-dark p-5 sm:p-6 lg:p-7 shadow-2xl border border-white/10 text-white">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => void loadData()}
                  className="flex items-center gap-1.5 px-3 py-2 bg-white border border-[#A5D6A7]/40 rounded-xl text-[12px] font-bold text-[#374151] hover:bg-[#F9FAFB]">
                  <RefreshCw size={13} /> {l('Refresh', 'புதுப்பி')}
                </button>
              </div>
            </div>

            <div className="flex gap-2 overflow-x-auto pb-2 border-b border-[#A5D6A7]/40">
              {[
                ...
                <button key={sub.id} onClick={() => setAnalyticsTab(sub.id)}
                  className={`px-4 py-2 rounded-xl text-[13px] font-bold whitespace-nowrap transition-colors ${analyticsTab === sub.id ? 'bg-[#111111] text-white' : 'bg-white border border-[#A5D6A7]/40 text-[#374151] hover:bg-[#F9FAFB]'}`}>
                  {sub.label}
                </button>
              ))}
            </div>
              ...
              <div className="bg-red-50 rounded-2xl border border-red-100 p-4 shadow-sm mb-4">
                <div className="flex items-center justify-between gap-2 mb-3">
                  ...
                  <button
                    ...
                    className="text-xs font-black text-red-700 hover:text-red-900 flex items-center gap-1 hover:underline"
                  >
                ...
                <div className="flex flex-wrap gap-2">
                  {lowStockProducts.map(p => (
                    <button
                      key={p.id}
                      onClick={() => setTab('inventory')}
                      className="bg-white border border-red-200 text-red-700 text-[11px] font-bold px-2.5 py-1 rounded-lg hover:bg-red-100/50 transition-colors"
                    >
            ...
            {/* Latest POS Bills */}
            <div className="bg-white rounded-2xl border border-[#A5D6A7]/30 p-5 shadow-sm">
              ...
              {latestPOS.length > 0 ? (
                <div className="overflow-x-auto rounded-xl border border-[#A5D6A7]/30">
        ...
        {tab === 'whatsapp' && (
          <div className="space-y-5">
                ...
                <div className="flex gap-1">
                  {(['all', 'today', 'week', 'month'] as const).map(preset => (
                    <button key={preset} type="button" onClick={() => applyAnalyticsPreset(preset)}
                      className={`px-2.5 py-1.5 rounded-lg text-[11px] font-black transition-colors ${analyticsDatePreset === preset ? 'bg-[#111111] text-white' : 'bg-[#F9FAFB] text-[#374151] hover:bg-[#A5D6A7]/40'}`}>
                      {preset === 'all' ? l('All','எல்லாம்') : preset === 'today' ? l('Today','இன்று') : preset === 'week' ? l('Week','வாரம்') : l('Month','மாதம்')}
                    </button>
                  ))}
                </div>
                <button onClick={() => void loadData()}
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-[#A5D6A7]/40 rounded-lg text-[11px] font-bold text-[#374151] hover:bg-[#F9FAFB]">
                        ...
                        return (
                          <React.Fragment key={order.id}>
                            <tr className={`hover:bg-blue-50/40 align-middle ${isExpanded ? 'bg-blue-50/30' : ''}`}>
                              ...
                              <td className="px-4 py-3 text-[#374151] whitespace-nowrap">{displayPhone(order.phone)}</td>
                                ...
                                <td colSpan={8} className="px-4 pb-5 pt-2 bg-blue-50/40">
                                  <div className="space-y-4">
                                    ...
                                    <div className="flex flex-wrap gap-4 text-[12px] bg-white rounded-xl p-3 border border-blue-100">
                                      ...
                                      <div><span className="font-black text-[#374151]">{l('Phone', 'தொலைபேசி')}: </span><span className="font-bold text-[#111111]">{displayPhone(order.phone)}</span></div>
                                    ...
                                    {its.length > 0 && (
                                      <div className="overflow-x-auto">
                                        <table className="w-full text-[12px] min-w-[540px] bg-white rounded-xl overflow-hidden border border-blue-100">
                                          ...
                                          <tfoot className="bg-[#F9FAFB] border-t border-[#A5D6A7]/30">
                                            <tr>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div className="bg-white rounded-2xl border border-[#A5D6A7]/30 p-4 shadow-sm">
          ...
          <div className="space-y-6">
              ...
              <div className="flex gap-2 overflow-x-auto md:flex-wrap md:overflow-visible no-scrollbar bg-[#F3F4F6] rounded-2xl p-1.5">
                {([
                  ...
                  <button key={id} onClick={() => setPosAnalyticsTab(id as PosAnalyticsTab)}
                    className={`px-4 py-2 rounded-xl text-[12px] font-black tracking-wide transition-all whitespace-nowrap ${posAnalyticsTab === id ? 'bg-white text-[#2E7D32] shadow-sm' : 'text-[#6B7280] hover:text-[#111111]'}`}>
                    {label}
                  </button>
                ))}
              </div>
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-4">
                  {[
                    ...
                    { label: 'TOTAL REVENUE',    helper: 'POS + Services',          value: formatCurrency(analytics.totalCompletedRevenue), icon: <RMIcon size={16} />,      color: 'text-blue-500',    bg: 'bg-blue-50' },
                ...
                <div className="bg-white rounded-2xl border border-[#A5D6A7]/30 p-5 shadow-sm">
                  ...
                  <div className="mb-3">
                    <input
                      ...
                      className="w-full px-4 py-2.5 bg-[#F9FAFB] border border-[#A5D6A7]/60 rounded-xl text-[13px] font-bold text-[#111111] placeholder:text-[#8A9384] focus:outline-none focus:border-[#2E7D32] transition-colors"
                    />
                  </div>
                  {(() => {
                ...
                {analytics.topCategories.length > 0 ? (
                  ...
                  <div className="space-y-3 md:hidden">
                    {analytics.topCategories.map((c, i) => (
                      <div key={c.name} className="rounded-2xl border border-[#A5D6A7]/30 bg-[#FBFAF6] p-4">
                  ...
                  <div className="hidden md:block overflow-x-auto rounded-xl border border-[#A5D6A7]/30">
            ...
            {posAnalyticsTab === 'coupons' && (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                  <div className="xl:col-span-2 bg-white rounded-2xl border border-[#A5D6A7]/30 p-5 shadow-sm">
                  ...
                  <div className="bg-white rounded-2xl border border-[#A5D6A7]/30 p-5 shadow-sm">
                    ...
                    <div className="space-y-3 md:hidden">
                      {analytics.topCoupons.map((coupon, i) => (
                        <div key={coupon.code} className="rounded-2xl border border-[#A5D6A7]/30 bg-[#FBFAF6] p-4">
                          <div className="flex items-start justify-between gap-3">
                            <div>
                              <p className="text-[13px] font-black text-[#9BAB9A]">#{i + 1}</p>
        ...
        {/* ── ORDER MANAGEMENT ── */}
        {tab === 'history' && (
          <div className="space-y-4 sm:space-y-6 rounded-[20px] sm:rounded-[28px] border border-[#A5D6A7]/60 bg-[#FBFAF6] p-3 sm:p-6 lg:p-7 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
            ...
            <div className="rounded-2xl border border-[#A5D6A7]/60 bg-white p-3 sm:p-6 shadow-sm">
              {/* Bill type filter */}
              <div className="flex flex-wrap gap-2 mb-4">
                {([
                  ...
                  <button key={v} type="button" onClick={() => { setBillTypeFilter(v); setTimeout(() => void runSearch(undefined, undefined, undefined, v), 0) }}
                    className={`min-h-[44px] px-3 py-1.5 rounded-xl text-[12px] font-black transition-colors ${billTypeFilter === v ? 'bg-[#111111] text-white shadow-sm' : 'bg-[#F9FAFB] text-[#374151] hover:bg-[#A5D6A7]/40'}`}>
                    {l}
                  </button>
                ))}
              </div>
                ...
                <div className="flex flex-wrap gap-2 items-center">
                  {(['today', 'week', 'month', 'custom'] as const).map(preset => (
                    <button key={preset} type="button" onClick={() => applyDatePreset(preset)}
                      className={`min-h-[44px] px-3 py-1.5 rounded-xl text-[12px] font-black transition-colors ${datePreset === preset ? 'bg-[#2E7D32] text-white shadow-sm' : 'bg-[#F9FAFB] text-[#374151] hover:bg-[#A5D6A7]/40'}`}>
              ...
              <div className="space-y-3 md:hidden">
                {filteredSearchResults.slice(0, 50).map(o => {
                  ...
                  return (
                    <div key={o.id} className="rounded-2xl border border-[#A5D6A7]/60 bg-[#FBFAF6] p-3 sm:p-4 space-y-3">
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                      ...
                      <div className="grid grid-cols-2 gap-2 sm:gap-3 text-[12px] sm:text-[13px]">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="font-semibold text-[#374151] truncate">{displayPhone(o.phone)}</p>
                        </div>
                        <div className="min-w-0">
                      ...
                      <div className="flex flex-col gap-2 pt-1">
                        ...
                        <div className="flex flex-row gap-2 w-full">
                          <button onClick={() => void openOrderInvoice(o, 'view')} className="inline-flex h-10 flex-1 items-center justify-center gap-1.5 rounded-xl border border-[#A5D6A7]/60 px-2 text-[12px] font-black text-[#111111] transition-colors hover:bg-white" title="View Invoice">
                            <Eye size={14} /> View
                          </button>
                          ...
                          {role === 'admin' && (
                            <button onClick={() => void deleteOrder(o.id, o.invoice_no)} className="h-10 w-10 shrink-0 rounded-xl border border-[#A5D6A7]/60 text-[#2E7D32] transition-colors hover:bg-[#2E7D32]/5" title="Delete Order">
                ...
                {filteredSearchResults.length === 0 && (
                  <div className="rounded-2xl border border-[#A5D6A7]/60 bg-[#FBFAF6] px-4 py-8 text-center text-[#374151]">{l('No matching bills', 'பில்கள் இல்லை')}</div>
                )}
              </div>
              <div className="hidden md:block overflow-x-auto rounded-xl border border-[#A5D6A7]/60 bg-[#FBFAF6]">
                <table className="w-full text-left text-[13px]">
                  <thead className="bg-[#F9FAFB] text-[10px] uppercase tracking-wider text-[#374151]">
                    <tr>
                  ...
                  <tbody className="divide-y divide-[#A5D6A7]/30 bg-white">
                    {filteredSearchResults.slice(0, 50).map(o => {
                      const billTypeLabel = normalizeOrderType(o.order_type) === 'manual_sale' ? 'MANUAL' : normalizeOrderMode(o.order_mode) === 'online' ? 'ONLINE' : 'OFFLINE'
                      const billTypeClass = normalizeOrderType(o.order_type) === 'manual_sale' ? 'bg-purple-50 text-purple-700' : normalizeOrderMode(o.order_mode) === 'online' ? 'bg-blue-50 text-blue-700' : 'bg-orange-50 text-orange-700'
                      return (
                        ...
                        <tr key={o.id} className="hover:bg-[#F9FAFB] text-center">
                          ...
                          <td className="whitespace-nowrap px-2 py-3 text-[11px] text-[#374151]">{displayPhone(o.phone)}</td>
                        ...
                        {historyExpandedId === o.id && (
                          <tr className="bg-[#F9FAFB] border-b border-[#A5D6A7]/40">
                            <td colSpan={12} className="px-4 py-4">
                              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-[12px]">
                                <div>
                                  <p className="text-[10px] font-black uppercase text-[#9BAB9A] tracking-wider mb-1">Staff / Handler</p>
        ...
        {tab === 'coupons' && (
          <div className="space-y-4">
            <div className="flex flex-col gap-3">
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-full border border-[#A5D6A7] bg-[#FBFAF6] px-3 py-2 text-[11px] font-black text-[#2E7D32] shadow-sm transition-colors hover:bg-[#F7F1E7]"
                >
                  <RefreshCw size={12} />
                  Refresh
                </button>
              ...
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                <div className="rounded-xl border border-[#A5D6A7] bg-[#FBFAF6] px-3 py-3 shadow-sm">
                ...
                <div className="rounded-xl border border-[#A5D6A7] bg-[#FBFAF6] px-3 py-3 shadow-sm">
                  ...
                  <p className="mt-1 text-[20px] font-black text-[#2E7D32]">{coupons.filter(c => c.is_active).length}</p>
                </div>
                <div className="rounded-xl border border-[#A5D6A7] bg-[#FBFAF6] px-3 py-3 shadow-sm">
            ...
            <div className="grid grid-cols-1 gap-4 xl:grid-cols-[380px_minmax(0,1fr)]">
              <form onSubmit={saveCoupon} className="rounded-2xl border border-[#A5D6A7] bg-[#FFFCF6] p-4 shadow-sm space-y-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
              ...
              <div className="rounded-2xl border border-[#A5D6A7] bg-[#FFFCF6] p-4 shadow-sm">
        ...
        {tab === 'settings' && (
          <div className="max-w-4xl mx-auto space-y-6 pb-12">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-2xl font-black text-[#111111]">{l('Store Configuration & Settings', 'கடை கட்டமைப்பு & அமைப்புகள்')}</h2>
                <p className="text-xs font-semibold text-[#6B7280] mt-1">{l('Manage shop information, contact details, branding, receipts, and system thresholds.', 'கடை தகவல், தொடர்பு விவரங்கள், ரசீது அமைப்புகள் மற்றும் இருப்பு வரம்புகளை நிர்வகிக்கவும்.')}</p>
              </div>
              <button
                type="button"
                onClick={() => void reloadSettings()}
                className="flex items-center gap-1.5 px-3 py-2 bg-white border border-[#D1D5DB] rounded-xl text-xs font-bold text-[#374151] hover:bg-[#F9FAFB]"
              >
                <RefreshCw size={13} /> {l('Reload', 'மீண்டும் ஏற்று')}
              </button>
            </div>

            {settingsSaveSuccess && (
              <div className="p-4 bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-xl text-xs font-bold flex items-center gap-2">
                <CheckCircle size={16} /> {settingsSaveSuccess}
              </div>
            )}
            {settingsSaveError && (
              <div className="p-4 bg-red-50 border border-red-200 text-red-700 rounded-xl text-xs font-bold flex items-center gap-2">
                <AlertCircle size={16} /> {settingsSaveError}
              </div>
            )}

            <form onSubmit={handleSaveSettings} className="space-y-6">
              {/* Card 1: Store & Business Details */}
              <div className="bg-white rounded-2xl border border-borderLight p-6 shadow-sm space-y-4">
                <h3 className="text-sm font-black uppercase tracking-wider text-[#2E7D32] flex items-center gap-2">
                  <Store size={16} /> {l('Business Information', 'வணிக விவரங்கள்')}
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[11px] font-black uppercase text-[#6B7280] mb-1.5">{l('Shop Name', 'கடை பெயர்')}</label>
                    <input
                      type="text"
                      className="w-full px-3.5 py-2.5 rounded-xl border border-[#D1D5DB] text-sm font-semibold focus:outline-none focus:border-[#2E7D32]"
                  ...
                  <div>
                    <label className="block text-[11px] font-black uppercase text-[#6B7280] mb-1.5">{l('Owner / Signatory Name', 'உரிமையாளர் பெயர்')}</label>
                    <input
                      type="text"
                      className="w-full px-3.5 py-2.5 rounded-xl border border-[#D1D5DB] text-sm font-semibold focus:outline-none focus:border-[#2E7D32]"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[11px] font-black uppercase text-[#6B7280] mb-1.5">{l('Contact Phone Numbers', 'தொடர்பு தொலைபேசி எண்கள்')}</label>
                    <input
                      type="text"
                      className="w-full px-3.5 py-2.5 rounded-xl border border-[#D1D5DB] text-sm font-semibold focus:outline-none focus:border-[#2E7D32]"
                  ...
                  <div>
                    <label className="block text-[11px] font-black uppercase text-[#6B7280] mb-1.5">{l('Store Email Address', 'மின்னஞ்சல் முகவரி')}</label>
                    <input
                      type="email"
                      className="w-full px-3.5 py-2.5 rounded-xl border border-[#D1D5DB] text-sm font-semibold focus:outline-none focus:border-[#2E7D32]"
                ...
                <div>
                  <label className="block text-[11px] font-black uppercase text-[#6B7280] mb-1.5">{l('Store Address (Invoices & Receipts)', 'கடை முகவரி (விலைப்பட்டியல் & ரசீதுகள்)')}</label>
                  <textarea
                    rows={2}
                    className="w-full px-3.5 py-2.5 rounded-xl border border-[#D1D5DB] text-sm font-semibold focus:outline-none focus:border-[#2E7D32]"
                ...
                <div>
                  <label className="block text-[11px] font-black uppercase text-[#6B7280] mb-1.5">{l('Instagram Handle', 'Instagram கணக்கு')}</label>
                  <input
                    type="text"
                    className="w-full px-3.5 py-2.5 rounded-xl border border-[#D1D5DB] text-sm font-semibold focus:outline-none focus:border-[#2E7D32]"
                    value={settingsForm.instagram}
                    onChange={e => setSettingsForm({ ...settingsForm, instagram: e.target.value })}
                    placeholder="selvakkodi_agro_service"
                  />
                </div>
              </div>

              {/* Card 2: Inventory Threshold & Billing Settings */}
              <div className="bg-white rounded-2xl border border-borderLight p-6 shadow-sm space-y-4">
                <h3 className="text-sm font-black uppercase tracking-wider text-[#2E7D32] flex items-center gap-2">
                  <Sliders size={16} /> {l('Billing & Inventory Thresholds', 'பில்லிங் & இருப்பு அமைப்புகள்')}
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
                  <div>
                    <label className="block text-[11px] font-black uppercase text-[#6B7280] mb-1.5">{l('Default Low Stock Threshold', 'குறைந்த இருப்பு வரம்பு')}</label>
                    <input
                      type="number"
                      min={0}
                      className="w-full px-3.5 py-2.5 rounded-xl border border-[#D1D5DB] text-sm font-semibold focus:outline-none focus:border-[#2E7D32]"
                      value={settingsForm.lowStockLimit}
                      onChange={e => setSettingsForm({ ...settingsForm, lowStockLimit: Number(e.target.value) })}
                      required
                    />
                    <p className="text-[10px] text-[#6B7280] mt-1">{l('Triggers automatic alerts and banners when product stock reaches or drops below this count.', 'தயாரிப்பு இருப்பு இந்த எண்ணிக்கைக்குக் கீழே செல்லும் போது தானியங்கி எச்சரிக்கையை ஏற்படுத்தும்.')}</p>
                  </div>
                  <div className="pt-2">
                    <label className="flex items-center gap-3 cursor-pointer select-none">
                      <input
                        type="checkbox"
                        className="h-5 w-5 rounded border-[#D1D5DB] text-[#2E7D32] focus:ring-[#2E7D32]"
                        checked={settingsForm.gstEnabled}
                        onChange={e => setSettingsForm({ ...settingsForm, gstEnabled: e.target.checked })}
                      />
                      <div>
                        <span className="text-xs font-black text-[#111111]">{l('Enable GST Billing in POS', 'POS-ல் ஜிஎஸ்டி பில்லிங்கை இயக்கு')}</span>
                        <p className="text-[10px] text-[#6B7280]">{l('When enabled, GST line items are computed on invoices.', 'இயக்கப்பட்டால், விலைப்பட்டியலில் ஜிஎஸ்டி கணக்கிடப்படும்.')}</p>
                      </div>
                    </label>
                  </div>
                </div>
              ...
              <div className="flex justify-end gap-3">
                <button
                  ...
                  className="px-6 py-3 bg-maroon-dark text-white rounded-xl text-xs font-black hover:bg-maroon transition-colors disabled:opacity-50"
                >
                  {settingsSaving ? l('Saving Settings...', 'சேமிக்கப்படுகிறது...') : l('Save Configuration', 'அமைப்புகளைச் சேமி')}
                </button>
              </div>
            </form>
        ...
        return (
          ...
          >
            <div className="flex max-h-[95vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl bg-[#F9FAFB] shadow-2xl">
              <div className="flex shrink-0 items-center justify-between border-b border-[#A5D6A7]/60 bg-white px-4 py-3 sm:px-6">
                <div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="inline-flex min-h-[40px] items-center gap-1.5 rounded-xl border border-[#A5D6A7]/70 px-3 text-xs font-black text-[#111111] hover:bg-[#F9FAFB]"
                  >
