...
export default function Attendance() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [staffRes, attendanceRes] = await Promise.all([
        ...
        attendanceService.fetchAttendanceByDate(selectedDate)
      ])
      ...
      if (attendanceRes.error) {
        ...
        attendanceRes.data.forEach((r: AttendanceRecord) => {
  ...
  useEffect(() => {
    ...
    const fetchReport = async () => {
      ...
      try {
        const { data, error } = await attendanceService.fetchAttendanceByMonth(reportMonth)
        if (error) {
  ...
  const markAttendance = async (staffId: string, status: string) => {
    ...
    const res = await attendanceService.markAttendance({ staff_id: staffId, date: selectedDate, status })
    if (res.error) {
      alert(res.error)
      void fetchData()
    }
  ...
  const handleSaveStaff = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingStaff) {
        ...
        const res = await staffService.createStaff({ ...payload, is_active: true })
        if (res.error) throw new Error(res.error)
      }
      setShowModal(false)
  ...
  const toggleStaffActive = async (member: Staff) => {
    await staffService.updateStaff(member.id, { is_active: !member.is_active })
    void fetchData()
  }
  ...
  return (
    <div className="p-4 sm:p-6 space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-2">
        <h1 className="text-2xl font-black text-[#111111] flex items-center gap-2"><Users size={24} className="text-[#2E7D32]" /> Attendance & Staff</h1>
      </div>
      ...
      {dbError && (
        <div className="bg-yellow-50 border border-yellow-200 text-yellow-800 p-4 rounded-xl flex items-start gap-3">
          ...
          <div>
            <p className="font-black text-sm">Unable to load attendance or staff data</p>
            <p className="text-[13px]">Please ensure your session is active and server database tables are configured.</p>
          </div>
        </div>
      )}
      ...
      <div className="flex gap-2 flex-wrap">
        {(['today', 'report', 'staff'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={`shrink-0 px-4 py-2 rounded-xl font-bold text-sm transition-colors ${tab === t ? 'bg-[#2E7D32] text-white' : 'bg-white border border-[#A5D6A7]/60 text-[#374151] hover:bg-orange-50'}`}>
      ...
      {tab === 'today' && (
        <div className="space-y-5">
          <div className="flex flex-wrap items-center justify-between gap-4 bg-white p-4 rounded-2xl border border-[#A5D6A7]/60 shadow-sm">
              ...
              <table className="w-full text-left">
                ...
                <tbody>
                  {loading ? (
                    ...
                    return (
                      <tr key={member.id} className="border-b border-[#A5D6A7]/30 hover:bg-[#FAFAFA]">
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full bg-[#FFF8F2] text-[#2E7D32] border border-[#A5D6A7] flex items-center justify-center font-black text-sm shrink-0 uppercase">{member.name.charAt(0)}</div>
        ...
        <div className="space-y-5">
          <div className="flex justify-end">
            <button onClick={() => { setEditingStaff(null); setForm({ name: '', role: '', phone: '', base_salary: '' }); setShowModal(true) }} disabled={dbError}
              className="bg-[#2E7D32] text-white px-4 py-2 rounded-xl text-sm font-bold flex items-center gap-2 hover:bg-[#1B5E20] disabled:opacity-50">
          ...
          <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl">
            ...
            <form onSubmit={handleSaveStaff} className="space-y-4">
              <div>
                ...
                <input type="text" value={form.name} onChange={e => setForm({...form, name: e.target.value})} className="w-full border border-[#A5D6A7]/60 p-2.5 rounded-xl text-sm font-bold outline-none focus:border-[#2E7D32]" required />
              </div>
              <div>
                <label className="block text-[10px] font-black uppercase text-[#374151] mb-1.5">Role / Job Title *</label>
                <input type="text" value={form.role} onChange={e => setForm({...form, role: e.target.value})} placeholder="e.g. Field Supervisor, Agronomist, Cashier" className="w-full border border-[#A5D6A7]/60 p-2.5 rounded-xl text-sm font-bold outline-none focus:border-[#2E7D32]" required />
              </div>
              <div>
                <label className="block text-[10px] font-black uppercase text-[#374151] mb-1.5">Phone Number</label>
                <input type="tel" value={form.phone} onChange={e => setForm({...form, phone: e.target.value})} placeholder="+91" className="w-full border border-[#A5D6A7]/60 p-2.5 rounded-xl text-sm font-bold outline-none focus:border-[#2E7D32]" />
              </div>
              <div>
                <label className="block text-[10px] font-black uppercase text-[#374151] mb-1.5">Base Salary (₹)</label>
                <input type="number" step="0.01" min="0" value={form.base_salary} onChange={e => setForm({...form, base_salary: e.target.value})} className="w-full border border-[#A5D6A7]/60 p-2.5 rounded-xl text-sm font-bold outline-none focus:border-[#2E7D32]" placeholder="0.00" />
              </div>
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" disabled={submitting} className="flex-1 bg-[#2E7D32] text-white p-3 rounded-xl font-bold text-sm hover:bg-[#1B5E20] disabled:opacity-50">{submitting ? 'Saving...' : 'Save Staff'}</button>
              </div>
            </form>
          </div>
        </div>
