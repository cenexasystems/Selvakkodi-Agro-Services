...
import { reviewService, type CustomerReview } from '../services/reviewService'
...
const REMEDY_MAP: Record<string, { emoji: string; bg: string; border: string }> = {
  ...
  'Agro Services':      { emoji: '🌾', bg: '#F0FDF4', border: '#4ADE80' },
  'Boutique Services':  { emoji: '🌾', bg: '#F0FDF4', border: '#4ADE80' },
...
const TESTIMONIALS = [
  {
    ...
    location: 'Chennai',
    rating: 5,
    text: 'Exceptional quality and friendly service. The seeds, crop protection, and farming guidance were top quality from start to finish.',
    initials: 'PK',
    color: '#2E7D32',
  },
  {
  ...
  },
  {
    ...
    location: 'Madurai',
    rating: 5,
    text: 'Reliable agricultural supplies at reasonable prices. Customer service through WhatsApp is very responsive.',
    initials: 'KS',
    color: '#5e8c72',
  },
  {
...
export default function Home() {
  ...
  // ── Fetch reviews from API (or localStorage fallback) ────
  const fetchReviews = useCallback(async () => {
    try {
      const { data, error } = await reviewService.fetchReviews({ limit: 20 })
      if (!error && data && data.length > 0) {
        setCustomerReviews(data)
      } else {
        const fallback = lsGetReviews()
        if (fallback.length > 0) setCustomerReviews(fallback)
      }
    } catch {
      const fallback = lsGetReviews()
      if (fallback.length > 0) setCustomerReviews(fallback)
  ...
  return (
    <div className="bg-[#F9FAFB] text-[#111111]">
            ...
            >
              <div className="bg-white rounded-2xl px-4 py-3.5
                ...
                <div>
                  ...
                  <p className="text-[10px] text-[#374151] font-medium mt-0.5">Agro Service</p>
                </div>
              </div>
            </motion.div>
            ...
            >
              <Link to={`/products?cat=${encodeURIComponent(c.name)}`} className="flex flex-col items-center group">
                ...
                <div className="w-full aspect-square rounded-2xl overflow-hidden relative
                  border border-[#A5D6A7]/30 shadow-sm
      ...
      {/* ═══ HEALTH CONCERNS ═══ */}
      <section id="concerns" className="bg-white py-16 sm:py-24 border-y border-[#A5D6A7]/30 scroll-mt-20">
      ...
      <section className="bg-white py-16 sm:py-24 border-y border-[#A5D6A7]/30">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          <AnimatePresence>
            {showForm && (
              <motion.form
                ...
                className="mb-10 bg-[#F9FAFB] rounded-2xl p-6 border border-[#A5D6A7]/40 overflow-hidden"
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-2.5 rounded-xl border-2 border-[#A5D6A7] focus:border-[#2E7D32] outline-none bg-white text-[13px]"
                ...
                <div className="mb-5">
                  ...
                  <textarea
                    ...
                    className="w-full px-4 py-2.5 rounded-xl border-2 border-[#A5D6A7] focus:border-[#2E7D32] outline-none bg-white text-[13px] resize-none"
                ...
                <div className="flex gap-3">
                  ...
                  <button type="button" onClick={() => { setShowForm(false); setReviewError('') }}
                    className="px-5 py-2.5 rounded-xl border-2 border-[#A5D6A7] text-[#374151] font-bold text-[13px] hover:bg-[#F9FAFB] transition-colors">
                    Cancel
                  </button>
                </div>
              </motion.form>
          ...
          {reviewsLoading && (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-5">
              {[1, 2, 3, 4].map(i => (
                <div key={i} className="bg-[#F9FAFB] rounded-2xl p-5 border border-[#A5D6A7]/30 animate-pulse">
                  <div className="h-3 bg-[#A5D6A7] rounded w-24 mb-4" />
                  <div className="h-2.5 bg-[#A5D6A7] rounded w-full mb-2" />
                  <div className="h-2.5 bg-[#A5D6A7] rounded w-4/5 mb-2" />
                  <div className="h-2.5 bg-[#A5D6A7] rounded w-3/5" />
                </div>
              ))}
            </div>
          )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {TESTIMONIALS.map((review, idx) => (
              <motion.div
                ...
                className="bg-[#F9FAFB] rounded-2xl p-5 sm:p-6 border border-[#A5D6A7]/30 flex flex-col gap-3.5 hover:shadow-md transition-shadow duration-300"
              >
                ...
                <div className="flex items-center gap-2.5 pt-3 border-t border-[#A5D6A7]/40">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 items-stretch">
          ...
          <a
            ...
            className="group relative rounded-[2rem] overflow-hidden
              border border-[#A5D6A7]/40 shadow-sm
          ...
          <div className="bg-white rounded-[2rem] p-8 sm:p-10
            ...
            <div>
              ...
              <p className="text-[#374151] text-[14px] leading-[1.7] font-medium mb-6">
                Visit {BRAND_EN} for agriculture and agro products. Our team will help you find the right option for your needs.
              </p>
