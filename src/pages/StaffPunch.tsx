...
export default function StaffPunch({ embedded = false }: StaffPunchProps) {
  ...
  useEffect(() => {
    const fetchStaffList = async () => {
      setLoading(true)
      const { data, error } = await staffService.fetchStaff({ activeOnly: true })
  ...
  const selectStaff = async (s: Staff) => {
    setSelectedStaff(s)
    setLoading(true)
    const { data } = await attendanceService.fetchStaffAttendanceForDate(s.id, today)
    setTodayRecord(data)
    setLoading(false)
    setStep('punch')
  }
  ...
  const punchIn = async () => {
    ...
    setSaving(true)
    setNotice('')
    const res = await attendanceService.punchAttendance(selectedStaff.id, 'punch_in')
    if (res.error) {
      setNotice(res.error)
    } else if (res.data) {
  ...
  const punchOut = async () => {
    ...
    setSaving(true)
    setNotice('')
    const res = await attendanceService.punchAttendance(selectedStaff.id, 'punch_out')
    if (res.error) {
      setNotice(res.error)
    } else if (res.data) {
      setTodayRecord(res.data.record)
    ...
    <div className={embedded ? 'p-4 sm:p-6 space-y-5' : 'flex-1 flex flex-col px-4 py-6 max-w-2xl mx-auto w-full'}>
      ...
      {step === 'select' && (
        <>
          <div className={embedded ? 'mb-5' : 'mb-6 text-center'}>
            {embedded ? (
              <div className="bg-white rounded-2xl border border-[#A5D6A7]/60 p-4 shadow-sm flex items-center justify-between mb-5">
                <div>
                  <p className="text-[10px] font-black uppercase tracking-widest text-[#2E7D32] mb-0.5">Attendance</p>
                ...
                <div className="text-right">
                  <p className="text-2xl font-black text-[#2E7D32]">{currentTime}</p>
                </div>
              </div>
            ) : (
              <>
                <p className="text-[12px] font-black uppercase tracking-widest text-[#2E7D32] mb-1">Today</p>
          ...
          {loading ? (
            <div className="flex justify-center py-16"><span className="h-10 w-10 animate-spin rounded-full border-4 border-[#A5D6A7] border-t-[#2E7D32]" /></div>
          ) : staff.length === 0 ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {staff.map(s => (
                <button key={s.id} onClick={() => void selectStaff(s)}
                  className="bg-white rounded-2xl border border-[#A5D6A7]/60 p-4 text-left shadow-sm hover:border-[#2E7D32]/40 hover:shadow-md active:scale-95 transition-all">
                  <div className="h-12 w-12 rounded-xl bg-[#E8F5E9] flex items-center justify-center mb-3">
                    <span className="text-2xl font-black text-[#2E7D32]">{s.name.charAt(0).toUpperCase()}</span>
                  </div>
      ...
      {step === 'punch' && selectedStaff && (
        <div className="flex flex-col items-center">
          <button onClick={reset} className="self-start flex items-center gap-1 text-[#2E7D32] font-bold text-sm mb-6">
          ...
          {loading ? (
            ...
            <div className="w-full max-w-sm mx-auto">
              <div className="bg-white rounded-2xl border border-[#A5D6A7]/60 p-6 shadow-sm mb-6 text-center">
                <div className="h-20 w-20 rounded-2xl bg-[#E8F5E9] flex items-center justify-center mx-auto mb-4">
                  <span className="text-4xl font-black text-[#2E7D32]">{selectedStaff.name.charAt(0).toUpperCase()}</span>
                </div>
              ...
              {todayRecord && (
                <div className="bg-white rounded-2xl border border-[#A5D6A7]/60 p-4 mb-5 space-y-2">
              ...
              {!todayRecord?.clock_in ? (
                <button onClick={() => void punchIn()} disabled={saving}
                  className="w-full bg-[#2E7D32] hover:bg-[#1B5E20] text-white rounded-2xl py-5 text-xl font-black shadow-lg shadow-green-700/30 active:scale-95 transition-all disabled:opacity-60 flex items-center justify-center gap-3">
      ...
      {step === 'done' && selectedStaff && (
        <div className="flex flex-col items-center justify-center py-10 text-center">
          <div className={`h-24 w-24 rounded-full flex items-center justify-center mx-auto mb-6 ${notice === 'punch_in' ? 'bg-green-100' : 'bg-emerald-100'}`}>
            {notice === 'punch_in' ? <LogIn size={40} className="text-green-600" /> : <LogOut size={40} className="text-emerald-700" />}
          </div>
          ...
          <p className="text-lg font-black text-[#2E7D32] mb-6">
          ...
          {todayRecord?.clock_in && todayRecord?.clock_out && (
            <div className="bg-white rounded-2xl border border-[#A5D6A7]/60 p-4 mb-6 w-full max-w-xs text-sm">
          ...
          <button onClick={reset}
            className="bg-[#2E7D32] text-white px-8 py-3 rounded-xl font-black hover:bg-[#1B5E20] active:scale-95 transition-all">
            Done
          </button>
        </div>
      )}
  ...
  return (
    <div className="min-h-screen bg-[#FAFDF6] flex flex-col">
      <div className="bg-[#2E7D32] px-5 py-4 flex items-center justify-between shadow-md">
        <div className="flex items-center gap-3">
          <img src="/logo.png" alt="Logo" className="h-9 w-9 rounded-xl object-contain bg-white p-0.5" onError={e => { (e.target as HTMLImageElement).style.display = 'none' }} />
          <div>
            <p className="text-white font-black text-base leading-tight">Selvakkodi Agro Service</p>
      ...
      {inner}
      <div className="text-center py-3"><p className="text-[10px] text-[#9BAB9A]">Powered by Selvakkodi Agro Service Billing System</p></div>
    </div>
  )
}
